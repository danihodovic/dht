...
def pr(repo_dir, github_token, gitlab_token, gitlab_url):
    ...
    if "github" in remote_url:
        ...
        click.echo(click.style(pull_request.html_url, fg="green", bold=True))
        ...
        merge_request = projects[0].mergerequests.create(
            {
        ...
        click.echo(
            click.style(merge_request.attributes["web_url"], fg="green", bold=True)
        )
